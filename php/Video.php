...
class Video {
	...
	public function __construct( array $args, array $origin_data = array(), object $oembed_data = null, \WP_Error $errors = null ) {
		...
		$this->errors      = $errors ? $errors : new \WP_Error();
	}
	...
	private function get_error_html() {
		...
		foreach ( $this->errors->get_error_codes() as $code ) {
			foreach ( $this->errors->get_error_messages( $code ) as $key => $message ) {
	...
	private function missing_attribute_check() {
		...
		// Old shortcodes
		if ( ! empty( $this->origin_data['from'] ) && 'create_shortcodes' === $this->origin_data['from'] ) {
	...
	private function oembed_html2src( $data ) {
		...
		if ( empty( $data->html ) ) {
			$this->errors->add( 'no-oembed-html', 'No oembed html' );
			return null;
		}
		...
		if ( empty( $matches[1] ) ) {
			$this->errors->add( 'no-oembed-src', 'No oembed src detected' );
			return null;
		}

		if ( 'TikTok' !== $data->provider_name && ! valid_url( $matches[1] ) ) {
			$this->errors->add( 'invalid-oembed-src-url', 'Invalid oembed src url detected:' . $matches[1] );
			return null;
		}
	...
	private function arg_mode() {
		...
		if ( 'normal' !== $this->mode &&
			...
			$this->errors->add( 'mode-not-avail', $err_msg );
			$this->mode = 'normal';
		}
	...
	private function detect_provider_and_id_from_url() {
		...
		if ( $input_provider &&
			...
			$this->errors->add( 'detect!=oembed', "Regex detected provider <code>{$this->provider}</code> did not match given provider <code>$input_provider</code>" );
		}
	...
	private function validate_url( $url, $argname ) {
		...
		if ( ! empty( $url ) && ! valid_url( $url ) ) {
			...
			$this->errors->add( $argname, $error_msg );
		}

		return $url;
	}

	private function validate_align( $align ) {
	...
	private function validate_aspect_ratio( $aspect_ratio ) {
