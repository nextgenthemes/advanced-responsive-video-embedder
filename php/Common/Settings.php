...
// phpcs:disable SlevomatCodingStandard.TypeHints
		foreach ( $this->sections as $slug => $name ) :
			...
			?>
				<a @click="showSection('
				<?php
				// phpcs:disable SlevomatCodingStandard.TypeHints echo esc_attr( $slug ); 
				?>
				')"
					class="
					<?php
					// phpcs:disable SlevomatCodingStandard.TypeHints echo esc_attr( $classes ); 
					?>
					"
					v-bind:class='{ "nav-tab-active": sectionsDisplayed["
					<?php
					// phpcs:disable SlevomatCodingStandard.TypeHints echo esc_attr( $slug ); 
					?>
					"] }'>
					<?php
// phpcs:disable SlevomatCodingStandard.TypeHints echo esc_html( $name ); ?>
				</a>
			<?php
...
// phpcs:disable SlevomatCodingStandard.TypeHints
		...
		<p v-show="onlySectionDisplayed !== 'debug'">
			...
			<img v-if='isSaving == true'
				...
				src='
				<?php
	// phpcs:disable SlevomatCodingStandard.TypeHints echo esc_url( get_admin_url() . '/images/wpspin_light-2x.gif' ); 
				?>
				'
	...
	// phpcs:disable SlevomatCodingStandard.TypeHints
		...
		private function print_paid_section_message() {
			...
			?>
		<div class="ngt-block" v-show="
			<?php
	// phpcs:disable SlevomatCodingStandard.TypeHints echo $v_show; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped 
			?>
		">
			<p>
			<?php
	// phpcs:disable SlevomatCodingStandard.TypeHints esc_html_e( 'You may already set options for addons but they will only take effect if the associated addons are installed.', 'advanced-responsive-video-embedder' ); ?>
			</p>
		</div>
			<?php
	// phpcs:disable SlevomatCodingStandard.TypeHints
		...
		<div class="ngt-block"
			...
			<?php
	// phpcs:disable SlevomatCodingStandard.TypeHints require_once __DIR__ . '/Admin/partials/debug-info.php'; ?>
		</div>
			<?php
	// phpcs:disable SlevomatCodingStandard.TypeHints
		}
...
// phpcs:disable SlevomatCodingStandard.TypeHints
	...
	// phpcs:disable SlevomatCodingStandard.TypeHints
		...
		<div class='wrap wrap--nextgenthemes'
			...
			<h2>
			<?php
	// phpcs:disable SlevomatCodingStandard.TypeHints echo esc_html( get_admin_page_title() ); 
			?>
			</h2>
				<?php
	// phpcs:disable SlevomatCodingStandard.TypeHints $this->print_settings_tabs(); ?>
